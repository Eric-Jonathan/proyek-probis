...
@section('custom_js')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Simple pseudo-random generator seeded by a string (to create consistent fake data based on selected filters)
            ...
            // Function to generate and update chart datasets
            function updateChartData() {
                ...
                // Construct a unique seed string based on filters
                let seed = period;
                ...
                if (period === 'day') {
                    ...
                    labels.forEach(() => {
                        newProposals.push(random(0, 5));
                        completedSurveys.push(random(0, 4));
                    });
                    ...
                    labels.forEach(() => {
                        newProposals.push(random(1, 10));
                        completedSurveys.push(random(0, 8));
                    });

                } else if (period === 'month') {
                    const monthVal = $('#pickerMonth').val();
                    const yearVal = $('#pickerMonthYear').val();
                    seed += monthVal + '-' + yearVal;
                    const random = getSeededRandom(seed);
                    
                    labels = ['Minggu 1', 'Minggu 2', 'Minggu 3', 'Minggu 4'];
                    labels.forEach(() => {
                        newProposals.push(random(3, 18));
                        completedSurveys.push(random(2, 15));
                    });

                } else if (period === 'year') {
                    const yearVal = $('#pickerYear').val();
                    seed += yearVal;
                    const random = getSeededRandom(seed);
                    
                    labels = ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Agt', 'Sep', 'Okt', 'Nov', 'Des'];
                    labels.forEach(() => {
                        newProposals.push(random(10, 50));
                        completedSurveys.push(random(8, 45));
                    });
                }

                // Update Chart JS instance
                dashboardChart.data.labels = labels;
                dashboardChart.data.datasets[0].data = newProposals;
                dashboardChart.data.datasets[1].data = completedSurveys;
